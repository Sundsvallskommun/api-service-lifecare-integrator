...
class PaymentResourceTest {
	...
	void getPayments() {
		// Mock
		final var response = PagedPaymentResponse.create()
		...
		when(familyCareServiceMock.getPayments(eq(MUNICIPALITY_ID), any(PeriodParameters.class))).thenReturn(response);
		...
		final var result = webTestClient.get()
			...
			.exchange()
			.expectStatus().isOk()
			.expectBody(PagedPaymentResponse.class)
			.returnResult()
			.getResponseBody();

		// Verification
		assertThat(result).isEqualTo(response);
		verify(familyCareServiceMock).getPayments(eq(MUNICIPALITY_ID), any(PeriodParameters.class));
	}
}
