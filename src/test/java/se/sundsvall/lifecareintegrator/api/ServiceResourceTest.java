...
class ServiceResourceTest {
	...
	void getServices() {
		// Mock
		final var response = PagedServiceResponse.create()
		...
		when(familyCareServiceMock.getServices(eq(MUNICIPALITY_ID), any(PeriodParameters.class))).thenReturn(response);
		...
		final var result = webTestClient.get()
			...
			.exchange()
			.expectStatus().isOk()
			.expectBody(PagedServiceResponse.class)
			.returnResult()
			.getResponseBody();

		// Verification
		assertThat(result).isEqualTo(response);
		verify(familyCareServiceMock).getServices(eq(MUNICIPALITY_ID), any(PeriodParameters.class));
	}
}
