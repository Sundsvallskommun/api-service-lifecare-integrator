...
class ExecutionResourceTest {
	...
	private static final String PATH = "/{municipalityId}/executions";
	...
	void getExecutions() {
		// Mock
		final var response = PagedExecutionResponse.create()
		...
		when(familyCareServiceMock.getExecutions(eq(MUNICIPALITY_ID), any(PeriodParameters.class))).thenReturn(response);
		...
		final var result = webTestClient.get()
			...
			.exchange()
			.expectStatus().isOk()
			.expectBody(PagedExecutionResponse.class)
			.returnResult()
			.getResponseBody();

		// Verification
		assertThat(result).isEqualTo(response);
		verify(familyCareServiceMock).getExecutions(eq(MUNICIPALITY_ID), any(PeriodParameters.class));
	}
}
