...
class InvestigationResourceTest {
	...
	private static final String PATH = "/{municipalityId}/investigations";
	...
	void getInvestigations() {
		// Mock
		final var response = PagedInvestigationResponse.create()
			.withInvestigations(List.of())
			.withMetaData(PagingMetaData.create().withPage(1).withLimit(20).withCount(0).withTotalPages(0).withTotalRecords(0));
		when(familyCareServiceMock.getInvestigations(eq(MUNICIPALITY_ID), any(PeriodParameters.class))).thenReturn(response);
		...
		final var result = webTestClient.get()
			...
			.exchange()
			.expectStatus().isOk()
			.expectBody(PagedInvestigationResponse.class)
			.returnResult()
			.getResponseBody();

		// Verification
		assertThat(result).isEqualTo(response);
		verify(familyCareServiceMock).getInvestigations(eq(MUNICIPALITY_ID), any(PeriodParameters.class));
	}
}
