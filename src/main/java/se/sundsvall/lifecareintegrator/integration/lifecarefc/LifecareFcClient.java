 ...
 * auth (and the {@code X-API-Key} header) is added globally by {@link LifecareFcConfiguration}, so it is not part of
 * these method signatures. The list reads share {@code startDate}/{@code endDate} (required) and optional
